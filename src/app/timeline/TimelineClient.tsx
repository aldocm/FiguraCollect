...
interface TimelineClientProps {
  ...
  seriesList: { id: string; name: string; brandIds: string[]; lineIds: string[] }[]
  brands: { id: string; name: string }[]
}

export default function TimelineClient({ lines, seriesList, brands }: TimelineClientProps) {
  ...
  const filteredSeries = useMemo(() => {
    return seriesList.filter(s => {
      const matchBrand = selectedBrand ? s.brandIds.includes(selectedBrand) : true
  ...
  const groupedFigures = figures.reduce((acc, figure) => {
    const year = figure.releaseDate ? new Date(figure.releaseDate).getFullYear().toString() : 'TBA'
        ...
        <div className="relative min-h-[500px]">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 opacity-50">
               {hasSearched ? (
                 <p className="text-xl">No se encontraron figuras con estos filtros.</p>
               ) : (
                 <div className="flex flex-col items-center gap-4">
                    <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center">
                        <Filter size={32} className="text-primary" />
                    </div>
                    <p className="text-xl">Selecciona filtros para generar la línea del tiempo.</p>
                 </div>
               )}
            </div>
          ) : (
                    ...
                    <div key={year} className="relative">
                        ...
                        <div className="sticky top-24 z-20 flex justify-start md:justify-center mb-8 pl-12 md:pl-0">
                             <div className="bg-background/80 backdrop-blur border border-primary/30 px-6 py-2 rounded-full shadow-[0_0_20px_rgba(225,6,44,0.2)]">
                                <span className="text-2xl font-title font-bold text-primary">{year}</span>
                             </div>
                        </div>
                            ...
                            {groupedFigures[year].map((figure, index) => {
                                ...
                                return (
                                    <motion.div 
                                        ...
                                        className={`relative flex flex-col md:flex-row items-center ${isLeft ? 'md:flex-row-reverse' : ''} gap-8 md:gap-0`}
                                    >
                                        {/* Content Side */}
                                        <div className="w-full md:w-1/2 pl-12 md:pl-0 md:px-12">
                                            <Link href={`/catalog/${figure.id}`} className="block group">
                                                <div className={`bg-uiBase/40 backdrop-blur-sm border border-white/10 rounded-2xl p-4 hover:bg-white/10 transition-all hover:border-primary/50 hover:shadow-[0_0_30px_-10px_rgba(225,6,44,0.3)] flex gap-4 items-center ${isLeft ? 'md:flex-row-reverse md:text-right' : ''}`}>
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className={`flex items-center gap-2 text-xs text-gray-400 mb-1 ${isLeft ? 'md:justify-end' : ''}`}>
                                                            <Calendar size={12} />
                                                            <span>
                                                                {figure.releaseDate 
                                                                    ? new Date(figure.releaseDate).toLocaleDateString('es-MX', { month: 'long', day: 'numeric' }) 
                                                                    : 'Fecha pendiente'}
                                                            </span>
                                                        </div>
